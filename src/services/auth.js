...
export const registerService = ({ phone, password, name, email, address, career} ) => new Promise(async (resolve, reject) => {
    try {
        ...
        const token = response[1] && jwt.sign({ id: response[0].id, phone: response[0].phone }, 'dkanskfsdkfsndfksnfsdf', { expiresIn: '2d' })
        resolve({
            err: token ? 0 : 2,
